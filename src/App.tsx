...
function Header({ muted, toggleSound, t }: { muted: boolean; toggleSound: () => void; t: Text }) {
  ...
  const light = location.pathname === '/academy/beginner'
  return <header className={`${styles.header} ${light ? styles.headerLight : ''}`}><Link className={styles.logo} to="/" aria-label="Ardjuna home"><img src={`${import.meta.env.BASE_URL}assets/images/icon.png`} alt="" /><span>ARJUNA</span></Link><button className={styles.menuToggle} onClick={() => setOpen(!open)} aria-expanded={open} aria-label="Toggle menu"><i /><i /></button><nav className={open ? styles.navOpen : ''} aria-label="Main navigation">{nav.map(([label, path]) => <NavLink key={path} to={path}>{label}</NavLink>)}<button className={styles.soundButton} onClick={toggleSound} aria-label={muted ? 'Включить звук' : 'Выключить звук'} aria-pressed={!muted}>{muted ? '🔇' : '🔊'}</button></nav></header>
}

function Home({ muted, toggleSound, t }: { muted: boolean; toggleSound: () => void; t: Text }) { return <main><section className={styles.hero}><video className={styles.heroVideo} autoPlay muted={muted} loop playsInline aria-label="Dota 2 cinematic background"><source src={`${import.meta.env.BASE_URL}assets/videos/dota-web-25.mp4`} type="video/mp4" /></video><div className={styles.heroContent}><div className={styles.heroTitle}><h1>{t.hero[0]}<br /><span>{t.hero[1]}</span></h1><p>{t.motto}</p><a className={styles.discover} href="#story">{t.discover} <span>↘</span></a></div></div><button className={styles.heroSound} onClick={toggleSound} aria-label={muted ? 'Включить звук видео' : 'Выключить звук видео'}>{muted ? '🔇' : '🔊'}</button></section><section className={styles.story} id="story"><div className={styles.storyGrid}><h2>{t.storyTitle}</h2><p>{t.story}</p></div></section></main> }
...
function Academy({ t }: { t: Text }) {
  ...
  return <main className={styles.academy}><div className={styles.academyIntro}><span className={styles.eyebrow}>{t.academyKicker}</span><h1>{t.academyTitle}</h1><p>{t.academyLead}</p></div><div className={styles.levelGrid}>{t.levels.map((level) => <Link className={styles.levelCard} to={`/academy/${level.slug}`} onClick={(event) => openLevel(event, level.slug)} key={level.slug}><span className={styles.levelNumber}>{level.number}</span><h2>{level.title}</h2><p>{level.text}</p><span className={styles.choosePath}>{t.learn} <b>→</b></span></Link>)}</div></main>
}
...
function LevelPage({ index, t }: { index: number; t: Text }) {
  ...
  if (index === 0) return <main className={styles.beginnerPage}><section className={styles.beginnerContent}><div className={styles.beginnerCopy}><span className={styles.beginnerEyebrow}>{t.academyTitle} / {level.number}</span><span className={styles.beginnerTag}>{level.tag}</span><h1>{level.page}</h1><p>{level.detail}</p><Link className={styles.beginnerBack} to="/academy">{t.back}</Link></div><div className={styles.beginnerVisual} aria-label="Invoker, Rubick and Puck"><i className={styles.decorativeColumn} />{sceneHeroes[0].map((hero, heroIndex) => <div className={styles.beginnerHero} key={hero}><span className={styles.beginnerPortrait} aria-hidden="true">👤</span><b>{heroIndex + 1}</b><small>{hero}</small></div>)}</div></section></main>
  const heroes = index === 1 ? sceneHeroes[0] : sceneHeroes[2]
  return <main className={`${styles.advancedPage} ${index === 1 ? styles.experiencedPage : styles.professionalPage}`}><section className={styles.advancedContent}><div className={styles.advancedCopy}><span className={styles.advancedEyebrow}>{t.academyTitle} / {level.number}</span><span className={styles.advancedTag}>{level.tag}</span><h1>{level.page}</h1><p>{level.detail}</p><Link className={styles.advancedBack} to="/academy">{t.back}</Link></div><div className={styles.advancedVisual} aria-label={heroes.join(', ')}>{index === 1 ? <><i className={styles.pulseRing} /><i className={styles.pulseRing} /></> : <><span className={styles.floatingBook}>📖</span><span className={styles.floatingBook}>📖</span></>}{heroes.map((hero, heroIndex) => <div className={styles.advancedHero} key={hero}><span className={styles.advancedPortrait}><b>{hero}</b></span><strong>{heroIndex + 1}</strong><small>{hero}</small></div>)}</div></section></main>
}
