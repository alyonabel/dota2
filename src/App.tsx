...
const copy = {
  ru: {
    ...
    academyKicker: '03 / Выберите свой путь', academyTitle: 'АКАДЕМИЯ', academyLead: 'Знание превращает хаос боя в систему. Выберите свой уровень — и начните путь к следующему рангу.', academyHelp: 'Не знаете свой уровень?', academyTest: 'Пройдите короткий тест →',
    levels: [
      ...
      { slug: 'professional', number: '03', title: 'Продвинутый', tag: 'Путь к мастерству', text: 'Отточите командное взаимодействие, драфты и мышление высокого уровня.', page: 'ИГРАЙ НА ПРЕДЕЛЕ', detail: 'Углублённая подготовка: драфты, темп, коммуникация и анализ соперника. Для игроков, которые ставят перед собой самые высокие цели.' }
    ],
  ...
  en: {
    ...
    academyKicker: '03 / Choose your path', academyTitle: 'ACADEMY', academyLead: 'Knowledge turns the chaos of battle into a system. Choose your level and begin the climb to your next rank.', academyHelp: 'Not sure of your level?', academyTest: 'Take the short test →',
    levels: [
      ...
      { slug: 'professional', number: '03', title: 'Advanced', tag: 'Path to mastery', text: 'Refine teamwork, drafting, and high-level competitive thinking.', page: 'PLAY AT YOUR LIMIT', detail: 'Advanced preparation: drafting, tempo, communication, and opponent analysis. For players pursuing the highest goals.' }
    ],
...
function Academy({ t }: { t: Text }) {
  ...
  return <main className={styles.academy}><div className={styles.academyMap} aria-hidden="true"><span>KNOWLEDGE / SYSTEM</span><span>03.01</span></div><div className={styles.academyIntro}><span className={styles.eyebrow}>{t.academyKicker}</span><h1>{t.academyTitle}</h1><p>{t.academyLead}</p></div><div className={styles.levelGrid}>{t.levels.map((level, index) => <Link className={styles.levelCard} data-level={level.number} to={`/academy/${level.slug}`} onClick={(event) => openLevel(event, level.slug)} key={level.slug}><div className={styles.levelMeta}><span className={styles.levelNumber}>{level.number}</span><span>PATH / 0{index + 1}</span></div><svg className={styles.rankSymbol} viewBox="0 0 56 56" aria-hidden="true"><path d="M28 5 51 28 28 51 5 28Z" /><path className={index > 0 ? styles.rankVisible : ''} d="M28 15 41 28 28 41 15 28Z" /><path className={index > 1 ? styles.rankVisible : ''} d="M28 22 34 28 28 34 22 28Z" /></svg><span className={styles.levelTag}>{level.tag}</span><h2>{level.title}</h2><p>{level.text}</p><span className={styles.cardArrow} aria-hidden="true">→</span></Link>)}</div><Link className={styles.levelHelp} to="/academy#level-test" title="Coming soon"><span>{t.academyHelp}</span><strong>{t.academyTest}</strong></Link></main>
}
